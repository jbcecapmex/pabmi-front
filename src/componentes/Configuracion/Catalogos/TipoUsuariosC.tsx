...
export interface TiposUsuariosInterface {
  uuid: string;
  Cve: string;
  Nombre: string;
  Descripcion: string;
...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardatiposusuarios",
        headers : {
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipo Usuario Creado Exitosamente",
          });
          getAllTiposUsuarios();
        })
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminatiposusuarios",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
          ...
          .then(function (response) {
            Toast.fire({
              icon  : "success",
              title : "Tipo Usuario Eliminado Exitosamente",
            });
            getAllTiposUsuarios();
          })
          .catch(function (error) {
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizatiposusuarios",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipos Usuarios Actualizado Exitosamente",
          });
          getAllTiposUsuarios();
        })
        .catch(function (error) {
  ...
  // aqui es el consumo del endpoint para obtener el listado de roles de la base de datos
  const getAllTiposUsuarios = () => {
    axios({
      method    : "get",
      url       : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/obtienetiposusuarios",
      headers   : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
      },
      ...
      .catch(function (error) {
        ...
        }).then((r) => navigate("/Configuracion/Usuarios/TiposUsuarios"));
      });
  };
    ...
    <Grid container sx={{ }}>
      <Grid sx={{}} item xs={12}>
        ...
        <Link underline="hover" color="inherit" href="/Inicio">
          ...
          <Typography color="text.primary">Catálogo de Tipos de usuarios</Typography>
        </Breadcrumbs>
      </Grid>
      ...
      <Typography  variant="h5" sx={{ paddingTop:"1%", paddingLeft:"1%" }}> Catálogo de Tipos de Usuarios</Typography>  
