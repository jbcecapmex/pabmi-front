...
export interface TiposClasificacionInterface {
  uuid: string;
  Cve: string;
  Nombre: string;
  Descripcion: string;
...
export default function TiposClasificacion() {
...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardatiposclasificacion",
        headers : {
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipo Clasificación Creado Exitosamente",
          });
          getAllTipoClasificacion();
        })
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminatiposclasificacion",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
          ...
          .then(function (response) {
            Toast.fire({
              icon  : "success",
              title : "Tipo Clasificación Eliminado Exitosamente",
            });
            getAllTipoClasificacion();
          })
          .catch(function (error) {
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizatiposclasificacion",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipo Clasificación Actualizado Exitosamente",
          });
          getAllTipoClasificacion();
        })
        .catch(function (error) {
  ...
  const getAllTipoClasificacion = () => {
    axios({
      method    : "get",
      url       : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/obtienetiposclasificacion",
      headers   : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
      },
      ...
      .catch(function (error) {
        ...
        }).then((r) => navigate("/Configuracion/Catalogos/TiposClasificacion"));
      });
  };
                    ...
                    <Grid item xs={12}>
                      <Box>
                        <Typography variant="h5" sx={{ padding: "1%" }}>
                          Detalle de Tipo de Clasificación
                        </Typography>
                      </Box>
                    </Grid>
