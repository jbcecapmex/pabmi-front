...
export interface TipoDependenciasInterface {
  uuid: string;
  Cve: string;
  Nombre: string;
  Descripcion: string;
...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardatiposdependencias",
        headers : {
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminatiposdependencias",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizatiposdependencias",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
  ...
  const getAllTipoDependencias = () => {
    axios({
      ...
      .catch(function (error) {
        ...
        }).then((r) => navigate("/Configuracion/Catalogos/TipoDependencias"));
      });
  };
    ...
    <Grid container sx={{ }}>
      <Grid sx={{}} item xs={12}>
        ...
        <Link underline="hover" color="inherit" href="/Inicio">
          ...
          <Typography color="text.primary">Catálogo de Tipo Dependencias</Typography>
        </Breadcrumbs>
      </Grid>
