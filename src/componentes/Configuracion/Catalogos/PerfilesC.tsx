...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardaperfiles",
        headers : {
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminaperfiles",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizaperfiles",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
  ...
  const getAllPerfiles = () => {
    axios({
      method    : "get",
      url       : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/obtieneperfiles",
      headers   : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
      },
