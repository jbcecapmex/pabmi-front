...
export interface TipoReporteInterface {
  uuid: string;
  Cve: string;
  Nombre: string;
  Descripcion: string;
...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardatiposreportes",
        headers : {
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipo Reporte Creado Exitosamente",
          });
          getAllTipoReporte();
        })
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminatiposreportes",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
          ...
          .then(function (response) {
            Toast.fire({
              icon  : "success",
              title : "Tipo Reporte Eliminado Exitosamente",
            });
            getAllTipoReporte();
          })
          .catch(function (error) {
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizatiposreportes",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
        ...
        .then(function (response) {
          ...
          Toast.fire({
            icon  : "success",
            title : "Tipo Reporte Actualizado Exitosamente",
          });
          getAllTipoReporte();
        })
        .catch(function (error) {
  ...
  const getAllTipoReporte = () => {
    axios({
      ...
      .catch(function (error) {
        ...
        }).then((r) => navigate("/Configuracion/Catalogos/TipoReporte"));
      });
  };
    ...
    <Grid container sx={{ }}>
      <Grid sx={{}} item xs={12}>
        ...
        <Link underline="hover" color="inherit" href="/Inicio">
          ...
          <Link underline="hover" color="inherit" href="/Configuracion/Catalogos/TipoReporte">
            Configuración
          </Link>
          <Link underline="hover" color="inherit" href="/Configuracion/Catalogos/TipoReporte">
            Catálogos
          </Link>
          <Typography color="text.primary">Catálogo de Tipo de Reportes</Typography>
        </Breadcrumbs>
      </Grid>
