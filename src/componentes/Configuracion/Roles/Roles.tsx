...
import {Edit as EditIcon, Add as AddIcon, Delete as DeleteIcon,} from "@mui/icons-material";import { useEffect, useState } from "react";
import axios from "axios";
import {Box,Breadcrumbs,Button,Card,CardContent,CardHeader,FormControl,Grid,IconButton,InputLabel,Link,OutlinedInput,TextField,Tooltip,Typography,} from "@mui/material";
...
const handleClose = ()  => setOpen(false);
  ...
  const handleSave = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/guardaroles",
        headers : {
  ...
  const handleDelete = (event: any, cellValues: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios({
          method    : "post",
          url       : process.env.REACT_APP_APPLICATION_ENDPOINT +"/catalogos/eliminaroles",
          headers   : {
                        "Content-Type": "application/json",
                        Authorization: localStorage.getItem("jwtToken") || "",
          },
...
// Handle update
  const handleUpdate = () => {
    if (cve === "" || nombre === "" || descripcion === ""){
      ...
      axios({
        method  : "post",
        url     : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/actualizaroles",
        headers : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
        },
  ...
  const getAllRoles = () => {
    axios({
      method    : "get",
      url       : process.env.REACT_APP_APPLICATION_ENDPOINT + "/catalogos/obtieneroles",
      headers   : {
                    "Content-Type": "application/json",
                    Authorization: localStorage.getItem("jwtToken") || "",
      },
          ...
          <Card sx={{ p: 0, boxShadow: 8, height: "86vh" }}>
            <CardContent sx={{ fontFamily: "MontserratBold", bgcolor: "" }}>
              ...
              {/* <MUIXDataGrid id={Math.random} columns={columns} rows={rows} /> */}
			        <MUIXDataGrid id={(row: any) => row.Id} columns={columns} rows={rows}/>
              {/* AGREGAR---------------------------------------------------------------------------------------------------------------------------------------------- */}
